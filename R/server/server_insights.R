...
#' Generate Free Text Insights
#'
#' Generates insights for the Free Text tab including response patterns,
...
#' @return A list of insight objects with type and content
#' @export
generate_free_text_insights <- function(df, free_text_questions) {
  if (is.null(df) || nrow(df) == 0 || length(free_text_questions) == 0) {
    return(list(
      list(type = "Info", content = "No data available for insights.")
    ))
  }
  ...
  response_counts <- sapply(free_text_questions, function(col) {
    if (col %in% names(df)) {
  ...
  if (total_responses > 0) {
    insights <- c(insights, list(list(
      type = "Key Finding",
      content = paste0("Students provided ", total_responses, " free text responses across ", 
                       length(free_text_questions), " questions.")
    )))
    
    # Find most answered question
    most_answered_idx <- which.max(response_counts)
    most_answered_col <- names(response_counts)[most_answered_idx]
    most_answered_count <- response_counts[most_answered_idx]
    most_answered_pct <- round(most_answered_count / nrow(df) * 100, 1)
    
    insights <- c(insights, list(list(
      type = "Notable Pattern",
      content = paste0("The most answered free text question was '", 
                       substr(get_column_display_name(most_answered_col), 1, 50), 
                       "' with ", most_answered_pct, "% response rate.")
    )))
    
    # Analyze word counts for the most answered question
    if (most_answered_col %in% names(df)) {
      responses <- df[[most_answered_col]]
      responses <- responses[!is.na(responses) & responses != ""]
      
      if (length(responses) > 0) {
        word_counts <- sapply(responses, function(text) {
          length(strsplit(text, "\\s+")[[1]])
        })
        
        avg_words <- round(mean(word_counts), 1)
        median_words <- round(median(word_counts), 1)
        
        insights <- c(insights, list(list(
          type = "Notable Pattern",
          content = paste0("Average response length is ", avg_words, " words (median: ", 
                           median_words, " words).")
        )))
        
        # Check for very short responses
        short_responses <- sum(word_counts < 10)
        if (short_responses > 0) {
          short_pct <- round(short_responses / length(responses) * 100, 1)
          insights <- c(insights, list(list(
            type = "Recommendation",
            content = paste0(short_pct, "% of responses are very short (<10 words). ", 
                             "Consider prompting for more detailed feedback.")
          )))
        }
      }
    }
    
    # Sentiment analysis for first question with responses
    for (col in free_text_questions) {
      if (col %in% names(df)) {
        responses <- df[[col]]
        responses <- responses[!is.na(responses) & responses != ""]
        ...
        if (length(responses) > 0) {
          ...
          positive_words <- c("good", "great", "excellent", "helpful", "useful", "enjoy", 
                             "love", "like", "appreciate", "thank", "thanks", "positive")
          negative_words <- c("bad", "poor", "difficult", "hard", "confusing", "frustrating", 
                             "disappointed", "unhelpful", "hate", "dislike", "negative")
          
          sentiment_scores <- sapply(responses, function(text) {
          ...
          neutral_count <- sum(sentiment_scores == 0)
          ...
          if (positive_count + negative_count > 0) {
            ...
            insights <- c(insights, list(list(
              type = "Key Finding",
              content = paste0("Free text responses show ", dominant_sentiment, 
                               " sentiment (", positive_count, " positive, ", 
                               negative_count, " negative, ", neutral_count, " neutral).")
            )))
          }
          
          break  # Only analyze first question
        }
      }
    }
  } else {
    insights <- c(insights, list(list(
      type = "Info",
      content = "No free text responses were provided."
    )))
  }
